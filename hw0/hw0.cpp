...
int main() {
	...
	*/
    for (double d2 = 0; d2 <= 2; d2 += 0.1) {
        robot->_q << 0, d2, 0; // Joint 1,2,3 Coordinates (radians, meters, radians)
        ...
        */
        ...
        myfile << g.transpose().coeff(0,0) << "," << g.transpose().coeff(1,1) << "," << g.transpose().coeff(2,2) << endl;
